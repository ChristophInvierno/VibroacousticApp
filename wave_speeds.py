...
def wave_speeds( ElasticModulusData, ShearModulusData, PoissonRatiosData, MaterialPropertiesData, GeometryPropertiesData, Isotrop, Function, Argument):
    ...
    for i in range(Subs):
        ...
        nu21[i] = np.array( PoissonRatiosData[i][0] )
        nu13[i] = np.array( PoissonRatiosData[i][1] )
        nu23[i] = np.array( PoissonRatiosData[i][2] )
    ...
    if Isotrop==True: #ISO
        ...
        for i in range(Subs):
            ...
            D_nu[i] = 1-nu12[i]*nu21[i]-nu13[i]*nu31[i]-nu23[i]*nu32[i]-2*nu12[i]*nu32[i]*nu13[i]            
            ...
            fR_B_1[i] = c_B_shear_1[i,0]**2/(2*math.pi)*np.sqrt((rho[i]*d[i])/B1[i])     #Grenzfrequenz nach Meier 2000
            fR_g_1[i] = fR_B_1[i]/4
    ...
    Function.append(c_B.tolist())
    Function.append(c_B_eff.tolist())
    Function.append(c_B_eff_1.tolist())
    Function.append(c_B_eff_2.tolist())

    Argument.append(f_thickmode_long.tolist())
    Argument.append(f_thickmode_shear.tolist())
    Argument.append(f_thickmode_shear_x.tolist())
    Argument.append(f_thickmode_shear_y.tolist())

    #return c_B, c_B_eff, c_B_eff_1, c_B_eff_2, f_thickmode_long, f_thickmode_shear, f_thickmode_shear_x, f_thickmode_shear_y
    return c_L, c_S, c_B_eff, c_g_eff
